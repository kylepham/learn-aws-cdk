import { DynamoDBClient, PutItemCommand, ConditionalCheckFailedException } from "@aws-sdk/client-dynamodb";
import { APIGatewayProxyResult, EventBridgeHandler } from "aws-lambda";

const ddbClient = new DynamoDBClient({
  region: "us-west-1",
});

export const handler: EventBridgeHandler<
  string,
  {
    flightId: string;
    seats: string[];
    username: string;
  },
  APIGatewayProxyResult
> = async (event) => {
  console.log("[register-booking] Event:", event);

  if (!event.detail) {
    return {
      statusCode: 400,
      body: JSON.stringify({ message: "No data provided" }),
    };
  }

  try {
    const { flightId, seats } = event.detail;

    console.log("flightId:", flightId);
    console.log("seats:", seats);

    if (!flightId || !seats || !Array.isArray(seats)) {
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "Invalid data format" }),
      };
    }

    for (const seatId of seats) {
      const params = {
        TableName: "Seat",
        Item: {
          SeatId: { S: seatId },
          isBooked: { S: "true" },
          FlightId: { S: flightId },
        },
      };

      try {
        await ddbClient.send(new PutItemCommand(params));
      } catch (error) {
        if (error instanceof ConditionalCheckFailedException) {
          console.error(`Seat ${seatId} is already booked for flight ${flightId}`);
          return {
            statusCode: 500,
            body: JSON.stringify({
              message: `Seat ${seatId} is already booked for flight ${flightId}`,
            }),
          };
        }
        throw error;
      }
    }

    return {
      statusCode: 200,
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Headers": "*",
      },
      body: JSON.stringify({ message: "Registered booking successfully" }),
    };
  } catch (error) {
    console.error("Error registering seats:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({ message: "Internal error occurred" }),
    };
  }
};
